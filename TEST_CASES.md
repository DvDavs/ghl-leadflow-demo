...
**Eleven scenarios pass, and all eleven cover the currently deployed
artifacts: TC-01, TC-02, TC-02b, TC-03, TC-09, TC-10, TC-11, TC-12, TC-17,
TC-18 and TC-19.**
...
**And it demoted TC-03 in the same motion — for one session.** GHL v13 added
an `If/Else` on the `Opportunity Found` branch, the exact branch TC-03
exercises, so TC-03's v12 pass stopped covering the deployed workflow. **TC-03
was re-run against v13 the same night and is back**, on a brand-new fixture and
with the arm that had never been traversed: `inquiry_count` went `1 → 2`, which
only the `Increment` side of the `If/Else` can produce, and the opportunity
still came back from `Follow-up` to `Contacting` — so the `Go To` reached the
second `Find Opportunity`. That construct is no longer resting on a build
report. **TC-01 went the other way and got stronger.** Its GHL-side assertions
ride on the `Not Found` branch, which v13 left untouched, and TC-02b's first
submission re-observed them at v13; the same submission's n8n execution 72
re-observed its `leads_backup` row, ledger `completed` and `outcome=processed`
on the current active `4ab773e2`. TC-01 remains the only row covering both legs
at their deployed versions — TC-03's re-run deliberately proves the opposite of
an n8n event, that a re-inquiry fires no webhook at all.
...
| TC-03 | **Genuine re-inquiry** — same person, different intent, amplified rather than swallowed | PASS | GHL **v13** (2026-08-10 04:15), fixture `Rafael Okonkwo`. Exercises the `Increment` + `Go To` arm: `inquiry_count` `1 → 2` and `Follow-up → Contacting`. n8n ingress unchanged at **26 executions** — a re-inquiry fires no webhook. The v12 run is retained above it | [GHL](docs/evidence/ghl-tests.md#tc-03--genuine-re-inquiry-same-person-different-intent) |
...
its evidence file rather than buried: **the duplicate-opportunity guard has
still never been exercised** — TC-02b's re-run answered *why* rather than
closing it, and the answer is that at a 4.3 s separation the second run finds
the first's opportunity and never reaches `Create Opportunity` at all. TC-03's
re-run exercises it no more, for the same routing reason; **the error behind
...
matrix. **TC-03 was demoted on the same rule**, by the very edit that let
TC-02b be re-run, and it was restored the only way the rule allows — by running
it again against v13 on a fresh fixture, not by arguing the old evidence still
applied. The matrix costs a row every time the artifact moves; that is the rule
